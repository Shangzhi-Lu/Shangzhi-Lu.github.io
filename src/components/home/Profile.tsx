...
export default function Profile({ author, social, features, researchInterests }: ProfileProps) {
    ...
    useEffect(() => {
        ...
        const userHasLiked = localStorage.getItem('shangzhi-website-user-liked');
    ...
    const handleLike = () => {
        ...
        if (newLikedState) {
            localStorage.setItem('shangzhi-website-user-liked', 'true');
            setShowThanks(true);
            setTimeout(() => setShowThanks(false), 2000);
        } else {
            localStorage.removeItem('shangzhi-website-user-liked');
            setShowThanks(false);
        }
    };
    ...
    return (
        ...
        >
            {/* Profile Image */}
            <div className="w-72 h-80 max-w-full mx-auto mb-6 rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-[1.03]">
                <Image
                    src={author.avatar}
                    alt={author.name}
                    width={288}
